...
RSpec.describe TypeChecker do
  ...
  describe ".call" do
    it "validates if value matches the type" do
      expect(described_class.call(1, type: [:str, Integer])).to eq(true)
    end  
  ...
  describe ".base_type" do
    it "validates Integer type" do
      expect(described_class.base_type(1, Integer)).to eq(true)
    end
    
    it "validates built-in types" do
      aggregate_failures 'with one of valid types' do
        ['1', 1].each { |value| expect(described_class.base_type(value, [Integer, String])).to eq(true) }
      end
    end

    it "validates array of types" do
      expect(described_class.base_type(1, [Integer, :str, cstm_stub])).to eq(true)
    end

    it "returns false for invalid types" do
      expect(described_class.base_type("string", Integer)).to eq(false)
    end
    ...
    it "handles nil values" do
      expect(described_class.base_type(nil, Integer)).to eq(false)
    end

    it "handles custom class types" do
      custom_instance = cstm_stub.new
      aggregate_failures 'validating custom class types' do
        expect(described_class.base_type(custom_instance, cstm_stub)).to eq(true)
        expect(described_class.base_type(custom_instance, String)).to eq(false)
      end
    end
  end
  ...
  describe ".custom_type" do
    it "validates if value matches the custom type" do
      expect(described_class.custom_type(cstm_stub.new, cstm_stub)).to eq(true)
    end

    it "validates if value does not match the custom type" do
      expect(described_class.custom_type(1, cstm_stub)).to eq(false)
    end
  end 

  describe ".one_of" do
    it "validates if value matches any type in the array" do
      expect(described_class.one_of(1, [Integer, :str, cstm_stub])).to eq(true)
    end 

    it "validates if value does not match any type in the array" do
      expect(described_class.one_of("string", [Integer, :int, cstm_stub])).to eq(false)
    end
  end 
